...
async function updateClaude(root) {
  ...
  const original = await readText(file);
  // Claude Code resolves @path imports and loads them as always-on context.
  // The map is written on-demand, but agents were observed reaching straight
  // for repo-wide grep instead of opening it first despite the protocol
  // telling them to — auto-loading it here removes the need for the agent to
  // remember to check, at the cost of a bounded amount of always-on context.
